...
async function identify_user(agent)
{
  ...
  const act_num = agent.parameters.act_num;
  const client = new MongoClient(url,{useUnifiedTopology:true});
  await client.connect();
  const snap = await client.db("chatbot").collection("user_table").findOne({act_num: act_num});
  console.log(snap);
  if(snap==null){
...
function report_issue(agent)
{
  ...
  if(intent_val>4)
  {
    agent.add("Please enter the issue number from the available options");
  }
  else{
